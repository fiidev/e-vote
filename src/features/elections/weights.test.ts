...
describe("validateWeights", () => {
  it("valid saat semua role eligible punya bobot & total 100", () => {
    const error = validateWeights({ SISWA: 70, OSIS: 20, MPK: 10 }, [
      "SISWA",
      "OSIS",
      "MPK",
    ]);
  ...
  it("tolak saat total tidak 100", () => {
    expect(validateWeights({ SISWA: 50, OSIS: 20 }, ["SISWA", "OSIS"])).toBe(
  ...
  it("tolak saat role eligible tidak punya bobot", () => {
    expect(
      validateWeights({ SISWA: 80, OSIS: 20 }, ["SISWA", "OSIS", "MPK"]),
  ...
  it("toleransi floating point 100.0000001 tetap valid", () => {
    expect(
      validateWeights(
        { SISWA: 33.3333334, OSIS: 33.3333333, MPK: 33.3333333 },
        ["SISWA", "OSIS", "MPK"],
      ),
    ).toBeNull();
  });
...
describe("computeWeightedScore", () => {
  it("hitung skor sesuai formula Opsi B", () => {
    // 100 SISWA: 60 vote kandidat; 30 OSIS: 20 vote; bobot 70/30
    const score = computeWeightedScore(
      [
        { role: "SISWA", votesForCandidate: 60, totalEligible: 100 },
        { role: "OSIS", votesForCandidate: 20, totalEligible: 30 },
      ],
      { SISWA: 70, OSIS: 30 },
    );
  ...
  it("skip grup dengan totalEligible 0 (hindari NaN)", () => {
    const score = computeWeightedScore(
      [
        ...
        { role: "OSIS", votesForCandidate: 0, totalEligible: 0 },
      ],
      { SISWA: 80, OSIS: 20 },
    );
